...
import "../libraries/AppStorageCoupons.sol";
...
contract CouponToken is IERC165, IERC721,  IERC721Metadata, ModifierCoupons {
   ...
   /// @notice Constructor
    ...
    function safeTransferFrom(address _from, address _to, uint256 _tokenId, bytes calldata data) external override payable{
        require(s.lockingBlockhash == bytes32(0x0), "Trading has ended");
    ...
    // Call this function for registering/editing IPFS links containing coupon/token mappings readable by users
    function registerARaffleOutcome(uint256 _outcomeID, string calldata _outcome) external onlyOwner() {

        if(s.rafflingStorage.length < _outcomeID){
            s.rafflingStorage[_outcomeID] = _outcome;
        } else 
        {
               s.rafflingStorage.push(_outcome);
        }  

    }

    // Freewe the trading period, pick a raffle winner
    function drawRaffle() external onlyOwner() {

        s.lockingBlockhash = blockhash(block.number-1);

        emit TradingEnded(s.rafflingStorage[uint256(s.lockingBlockhash) % s.rafflingStorage.length]);
    }

    //Read an outcome at a specific position in the index
    function readAnOutcome(uint256 _outcomeID) external view returns(string memory){
    ...
    /// @notice Mint a token with a maximum tokenID of _maxSupply
    /// @dev Throws if `_maxTokenID` is not lower than current amount of token minted
    function mintFor(uint256 _maxSupply, address _receiver, string calldata _tokenURISuffix) external onlyOwner() {
        require(s.lockingBlockhash == bytes32(0x0), "Trading has ended");
        require(s.totalSupply < _maxSupply, "Max supply already minted");

        s.totalSupply++;
        s.internalBalanceOf[_receiver] = s.internalBalanceOf[_receiver] + 1;

        //Changing ownership
        s.ownerOfVar[s.totalSupply] = _receiver;

        //Emitting transfer event
        emit Transfer(address(0x0), _receiver, s.totalSupply);

        s.tokenURI_suffix[s.totalSupply] = _tokenURISuffix;
    }
